...
const fullPath = exports.fullPath = ( ...dirs ) => path.join( rootDir, ...dirs );
	...
	try {
		...
		for ( let name of Object.keys( localActions ) ) {
			...
			if ( typeof (action.js) === 'string' ) {
				...
				action.module.execAsync = promisify(
					action.module.execute.bind( action.module.execute ) );

				action.module.executeAsync = async params => {

					return { stdout : await action.module.execAsync ( params ) };

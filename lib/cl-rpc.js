...
RPC.prototype.parseParameter = function (parameter, cb) {
	...
	if ( !parameter.type ) return callback( "parameter \"" + parameter.name +
...
RPC.prototype.handleDependenciesMTime = async function ( callback ) {
...
const statAsync = ( Promise.promisify || util.promisify )( fs.stat );

RPC.prototype.addMTime = async function ( file, callback ) {
...
RPC.prototype.afterExecution = function(err, stdout, stderr, callback) {
	...
	var now = new Date();
	fs.utimes(libpath.join(rootDir, this.outputDirectory), now, now, function () {});

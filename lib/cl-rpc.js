...
function log ( message, filteredMessage ) {

	if ( emitLog ) return exports.emit( "log", filteredMessage || message );
...
exports.execute = async function ( POST, callback ) {
	...
	try {
		...
		msg.handle = handle;
		exports.emit( "action finished", msg );
		...
		log( 'Error ' );
		log( error );
		exports.emit( "action finished", { handle, error } );
...
RPC.prototype.execute = async function ( POST ) {
	...
	response.commandLine = this.commandLine.substr( 0, 500 );
	return response;
...
RPC.prototype.parseParameter = function ( parameter ) {
	...
	if ( ( value === undefined ) && ( parameter.alias ) )
		value = this.POST[ parameter.alias ];
	...
	if ( value === undefined ) {
		...
		if ( parameter.required ) throw "parameter " + name + " is required!";
		return;

	}

	if ( !parameter.type ) throw "parameter \"" + name +
		"\" needs a \"type\" field, please check JSON file";

	if ( !parameter.type.endsWith( "Array" ) ) value = value.toString();
	else {

		if ( !Array.isArray( value ) )
			throw "parameter " + name + " must be an array ";

		if ( parameter.length && ( parameter.length != value.length ) )
			throw "parameter" + name + " must be of length " + parameter.length;

	}

	this.filteredPOST[ name ] = value;
	let values;
	const separator = parameter.separator != undefined ? parameter.separator : " ";
	...
	case 'intArray':
		...
		if ( values.some( v => !Number.isInteger( v ) ) )
			throw 'one value in parameter ' + name + " is not an integer";
	...
	case 'numberArray':
		...
		values = value.map( parseFloat );

		if ( values.some( isNaN ) )
			throw 'one value in parameter ' + name + " is not a number";
	...
	case 'int':
		...
		if ( !Number.isInteger( intNumber ) )
			throw "parameter " + name + " must be an int";
	...
	case 'number':
		...
		if ( isNaN( floatNumber ) )
			throw "parameter " + name + " must be a number";
	...
	case 'flag' :
		...
		switch( value ){
			...
			default:
				...
				throw "parameter " + name + " must be a bool";
		}
...
RPC.prototype.getInputMTime = async function ( ) {
	...
	for ( let param of this.action.parameters ) {
		...
		const value = this.filteredPOST[ param.name ];
...
RPC.prototype.handleOutputDirectory = async function () {
	...
	if ( settings.permissions === 0 )  {
		this.POST.outputDirectory = this.POST.output_directory = undefined;
	}
	...
	case undefined :
		...
		const cacheRoot = this.POST.hasOwnProperty( "cacheRoot" ) ?
			this.POST.cacheRoot : "cache";
		dir = path.join( cacheRoot, hash.charAt(0), hash.charAt(1), hash );
		break;
...
RPC.prototype.isInCache = async function ( ) {
	...
	try {
		...
		if ( this.POST.forceCache )	this.log( "cache is forced" );
		else if ( inputMTime >  cachedMTime ) return false;
...
RPC.prototype.executeAction = async function () {
	...
	if ( runningAction ) {
		...
		exports.emit( "action started", this.POST );
		res = await promise;
		...
		if ( !res.err && logStream ) {
			...
			await Promise.all( [
				...
				fs.writeFile( cwd + "/action.json",
					JSON.stringify( this.filteredPOST, null, 2 ) )

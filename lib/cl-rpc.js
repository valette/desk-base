...
RPC.prototype.handleDependenciesMTime = async function ( ) {
	...
	await Promise.all( this.action.deps.map( d => this.addMTime( d ) ) );
...
RPC.prototype.addMTime = async function ( file ) {
	...
	try {
		...
		", check JSON of action '" + this.POST.action + "'";}
...
RPC.prototype.isInCache = async function ( ) {
	...
	for ( let [ key, value ] of Object.entries( this.action.output || {} ) ) {
		...
		params[ field ] = this.response[ field ] = path.join( this.outputDirectory, value );

	}

	for ( let { name } of this.action.parameters ) {
	...
	let stats;
	try { stats = await fs.stat( file ); } catch (e ) {}
	if ( !stats ) return false;
	const mtime = stats.mtime.getTime();

	if ( ( mtime  >= this.MTime ) && ( await fs.readFile( file )
		== this.parametersString ) ) {

		this.response.timeStamp = mtime;
		return true;
...
RPC.prototype.replayActionFromCache = async function () {
	...
	if ( this.POST.stdout ) promises.push(
		...
		fs.readFile( fullPath( this.outputDirectory, 'action.log' ) ),
		fs.readFile( fullPath( this.outputDirectory, 'action.err' ) )
	...
	if ( this.POST.stdout ) {
		...
		if ( stdout ) this.response.stdout = '' + stdout;
		if ( stderr ) this.response.stderr = '' + stderr;
...
RPC.prototype.executeAction = async function () {
	...
	if ( runningAction ) {
		...
		if ( !res.err && logStream ) {
			...
			await Promise.all( [
				...
				fs.writeFile( file,	this.parametersString )
	...
	this.response.timeStamp = res.timeStamp = res.timeStamp || now.getTime();

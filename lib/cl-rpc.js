...
function translate ( ...dirs ) { return path.join( rootDir, ...dirs ) }
	...
	try {
		...
		for ( let name of Object.keys( localActions ) ) {
			...
			if ( typeof (action.js) === 'string' ) {
				...
				action.module.executeAsync = util.promisify(
					action.module.execute.bind( action.module.execute ) );
...
RPC.prototype.executeAction = async function () {
	...
	const existingOpts = ongoingActions2[ this.hash ];
	...
	if ( existingOpts ) {
		...
		ongoingActions2[ this.hash ] = opts;
		...
		if ( this.outputDirectory ) {
			...
			logStream = fs.createWriteStream( path.join( rootDir,
				this.outputDirectory, "action.log" ) );
			logStream2 = fs.createWriteStream( path.join( rootDir,
...
RPC.prototype.afterExecution = async function( err, stdout, stderr ) {
	...
	delete ongoingActions2[ this.hash ];

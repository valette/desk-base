...
async function manageActions( POST ) {
	...
	case "kill" :
		...
		const opts = ongoingActions[ POST.actionHandle ];

		if ( !opts ) throw "handle to kill not found";

		log( "kill requested : " + POST.actionHandle );

		const proc = opts.RPC.childProcess;

		if ( !proc ) {

			opts.RPC.toKill = true;
...
RPC.prototype.execute = async function ( POST, callback ) {
	...
	try {
		...
		if ( this.outputDir ) this.log( 'in : ' + this.outputDir );
		...
		if ( await this.isInCache() ) {
			...
			this.afterExecution( await this.executeAction() );
...
RPC.prototype.executeAction = async function () {
	...
	if ( existingOpts ) {
		...
		this.childProcess = existingOpts.RPC.childProcess;
		const promise = new Promise ( res => { opts.callback = res; } );
		existingOpts.RPC.similarActions.push( opts );
		this.log( "same as action [" + existingOpts.RPC.id + "]" );
		return await promise;

	} else if ( typeof ( this.action.module ) === "object" ) {

		const params = Object.assign( JSON.parse( this.parametersString ),
			this.POSTFiles );

		params.output_directory =  fullPath( outputDirectory );
		const stdout = await this.action.module.executeAsync( params );
		return { stdout };
	...
	const promise = new Promise ( resolve => {
		...
		child = this.childProcess = exec( this.commandLine, commandOptions,
...
RPC.prototype.afterExecution = function( res ) {

...
var actionsDirectoriesQueue = async.queue( async function( task ) {
	...
	const data = await fs.readFile( counterFile );
	...
	await fs.mkdirs( libpath.join( rootDir, outputDirectory ) );
	await fs.writeFile( counterFile, JSON.stringify( { value : index } ) );
	return outputDirectory;

}, 1);

const RPC =  function ( POST, callback ) {

...
RPC.prototype.parseParameter = function ( parameter ) {
	...
	if ( !parameter.type.endsWith( "Array" ) ) value = value.toString();
		...
		if ( !Array.isArray( value ) )
			throw "parameter " + parameter.name + " must be an array ";

		if ( parameter.length && ( parameter.length != value.length ) )
			throw "parameter" + parameter.name + " must be of length " + parameter.length;
	...
	case 'intArray':
		...
		if ( values.some( v => !Number.isInteger( v ) ) )
			throw 'one value in parameter ' + parameter.name + " is not an integer";
	...
	case 'numberArray':
		...
		if ( values.some( isNaN ) )
			throw 'one value in parameter ' + parameter.name + " is not a number";
	...
	case 'number':
		...
		if ( isNaN( floatNumber ) )
			throw "parameter " + parameter.name + " must be a number";

...
RPC.prototype.isInCache = async function ( ) {
	...
	try {
		...
		const cachedMTime = cachedPOST.__MTime;
		if ( inputMTime >  cachedMTime ) return false;
		delete cachedPOST.__MTime;
...
RPC.prototype.executeAction = async function () {
	...
	const cwd = fullPath( outputDirectory );
	let res;
	const now = new Date();
	...
	if ( runningAction ) {
		...
		if ( !res.err && logStream ) {
			...
			logStream.end();
			logStream2.end();
			this.filteredPOST.__MTime = now.getTime();
	...
	res.status = res.status || 'OK (' + ( new Date().getTime() - now.getTime() ) / 1000 + 's)';
	return res;
